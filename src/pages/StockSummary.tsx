...
import { AlertTriangle, Search, TrendingDown, DollarSign, RefreshCw } from 'lucide-react';
...
export default function StockSummary() {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white p-8">
            ...
            {/* Table */}
            <div className="bg-slate-900/60 rounded-2xl border border-slate-800 overflow-hidden min-h-[400px]">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-40">
                        <RefreshCw className="animate-spin text-blue-500 mb-4" size={32} />
                        <p className="text-slate-500">Updating inventory data...</p>
                    </div>
                ) : (
                            ...
                            <tbody>
                                {filteredData.map((item, i) => (
                                    <tr key={i} className="border-t border-slate-800 hover:bg-slate-800/30">
                                        ...
                                        <td className="p-4 text-right font-bold">{Number(item.current_stock).toFixed(2)}</td>
                                        <td className="p-4 text-right text-emerald-400">₹{Number(item.stock_value).toLocaleString('en-IN')}</td>
